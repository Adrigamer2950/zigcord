pub fn GatewayClient(comptime Dispatcher: type) type {
    return struct {
        ...
        pub fn init(opts: GatewayClientOptions) @This() {
            return .{ .ws_client = opts.ws_client, .allocator = opts.allocator, .io = opts.io, .packet_lookup_interval = opts.packet_lookup_interval, .token = opts.token, .intents = opts.intents, .dispatcher = opts.dispatcher, .debug_options = opts.debug_options };
        }
        ...
        fn threadLoop(self: *@This()) !void {
            while (self.thread_should_continue.load(.acquire)) {
                ...
                const packets: [][]const u8 = self.ws_client.getAndDropInboundPackets(self.allocator) catch |err| {
                ...
                for (packets) |raw_data| {
                    ...
                    switch (op) {
                        .dispatch => {
                            ...
                            const dp_event = root.Dispatch.parseDispatch(packet_arena.allocator(), t_str, d) catch |err| {
